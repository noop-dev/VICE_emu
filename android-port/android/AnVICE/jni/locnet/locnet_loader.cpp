...
extern "C" void Java_com_locnet_vice_DosBoxLauncher_nativeStart(JNIEnv * env, jobject obj, jobject bitmap, jint width, jint height)
{
	...
	//const char * argv[] = { "dosbox", "-conf", "/sdcard/dosbox.conf" };
	const char * argv[] = {
			//locnet, use +core as dummy
			execute_file,
			"-chdir", execute_path,
			"-config", "/sdcard/sdl-vicerc",
			(loader_true_drive)?((loader_true_drive == -1)?"+truedrive":"-truedrive"):dummyArg,
			(loader_ntsc)?"-ntsc":dummyArg,
			(loader_border == 1)?"-sdllimitmode":dummyArg, (loader_border == 1)?"0":dummyArg,
			//"-mouse",
			//"-mousetype", "0", "-mouseport", "1", "-joydev2", "4",
			//"-sdlcustomw", "320", "-sdlcustomh", "200", "-sdllimitmode", "1",
			//"-soundrate", "22050",
