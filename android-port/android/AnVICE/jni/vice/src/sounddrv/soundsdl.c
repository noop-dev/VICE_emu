...
static void sdl_callback(void *userdata, Uint8 *stream, int len)
{
	...
	if ((!sdl_full) && (sdl_inptr == sdl_outptr)) {
		if (userdata) *(short *)userdata = 0;	//locnet
		return;
	}
	...
	//__android_log_print(ANDROID_LOG_INFO,"c64", "sdl_callback: %d", total);
			if (userdata) *(short *)userdata = len/sizeof(SWORD);	//locnet
            return;
        }
	...
	if (userdata) *(short *)userdata = total;	//locnet
}
...
//__android_log_print(ANDROID_LOG_INFO,"c64", "sdl_write2: %d, %d, %d, %d, %d", total, nr, sdl_inptr, sdl_outptr, sdl_len);
...
//__android_log_print(ANDROID_LOG_INFO,"c64", "not read");
			break;
		}
...
static int sdl_write(SWORD *pbuf, size_t nr)
{
    ...
    while (total < nr) {
        ...
        if (amount <= 0) {
            SDL_Delay(5);
			//__android_log_print(ANDROID_LOG_INFO,"c64", "sdl_write: overflow");
			//Android_AudioWriteBuffer();	//locnet
            continue;
        }
