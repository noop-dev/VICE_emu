...
static sound_t *residdtv_open(BYTE *sidstate)
{
    sound_t *psid;
    int	i;
...
static int residdtv_init(sound_t *psid, int speed, int cycles_per_sec)
{
    ...
    if (!psid->sid->set_sampling_parameters(cycles_per_sec, method,
        ...
        log_warning(LOG_DEFAULT,
                    "reSID-DTV: Out of spec, increase sampling rate or decrease maximum speed");
        return 0;
    }

    log_message(LOG_DEFAULT, "reSID-DTV: DTVSID, filter %s, sampling rate %dHz - %s",
...
static void residdtv_close(sound_t *psid)
{
    delete psid->sid;
    delete psid;
}

static BYTE residdtv_read(sound_t *psid, WORD addr)
{
    return psid->sid->read(addr);
}

static void residdtv_store(sound_t *psid, WORD addr, BYTE byte)
{
    psid->sid->write(addr, byte);
}

static void residdtv_reset(sound_t *psid, CLOCK cpu_clk)
{
    psid->sid->reset();
}

static int residdtv_calculate_samples(sound_t *psid, SWORD *pbuf, int nr,
                                      int interleave, int *delta_t)
{
    return psid->sid->clock(*delta_t, pbuf, nr, interleave);
}

static void residdtv_prevent_clk_overflow(sound_t *psid, CLOCK sub)
{
}

static char *residdtv_dump_state(sound_t *psid)
{
    return lib_stralloc("");
}

static void residdtv_state_read(sound_t *psid, sid_snapshot_state_t *sid_state)
{
    SIDDTV::State state;
    unsigned int i;
...
static void residdtv_state_write(sound_t *psid, sid_snapshot_state_t *sid_state)
{
    SIDDTV::State state;
    unsigned int i;
	...
	if (sid_state->exponential_counter_period[i])
        ...
        state.envelope_state[i] = (EnvelopeGeneratorDTV::State)sid_state->envelope_state[i];
        state.hold_zero[i] = (sid_state->hold_zero[i] != 0);
    }

    psid->sid->write_state((const SIDDTV::State)state);
}

sid_engine_t residdtv_hooks =
{
    residdtv_open,
    residdtv_init,
    residdtv_close,
    ...
    residdtv_store,
    residdtv_reset,
    residdtv_calculate_samples,
    residdtv_prevent_clk_overflow,
    residdtv_dump_state,
    residdtv_state_read,
    residdtv_state_write
};
