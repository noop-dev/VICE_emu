...
//LOG(("FLASHROM (locked) write %04x:%02x",addr,value));
	...
	{
		case 0:
			if((addr==0x1555)&(value==0xaa)) flash_unlock_count++;
			else flash_unlock_count=0;
		break;
		case 1:
			if((addr==0x0aaa)&(value==0x55)) flash_unlock_count++;
			else flash_unlock_count=0;
		break;
		case 2:
			if((addr==0x1555)&(value==0x80)) flash_unlock_count++;
			else flash_unlock_count=0;
		break;
		case 3:
			if((addr==0x1555)&(value==0xaa)) flash_unlock_count++;
