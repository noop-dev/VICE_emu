...
BYTE eeprom_data_readbit(void)
{
    ...
    if(bitpos==0)
    {
    	LOG(("EEPROM: eeprom_data_readbit[0x%04x]:0x%02x '%c'",eeprom_readpos>>3,value,value));	
    }
...
#if 0
void eeprom_data_write(BYTE value)
{
    static BYTE bits[8]={0x80,0x40,0x20,0x10,0x08,0x04,0x02,0x01};
//    static BYTE bits[8]={0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80};
//    LOG(("EEPROM: eeprom_data_write:%d",value));	
    if(value)
    {
    	eeprom_writeval|=bits[eeprom_writebit];
    }
    eeprom_writebit++;
    if(eeprom_writebit==8)
    {
	    LOG(("EEPROM: eeprom_data_write: %04x:%02x (%c)",eeprom_writepos,eeprom_writeval,eeprom_writeval));	
	    eeprom_writepos++;
	    eeprom_writeval=0;
	    eeprom_writebit=0;
    }
...
//LOG(("eeprom CMDA0 pos %02x (%02x)",eeprom_cmdbuf[0],eeprom_cmdbuf[1]));
				...
				default:
					if(eeprom_mode!=EEPROM_INCA0_DATA) 
						break;
LOG(("EEPROM: CMDA0 write byte (%02x)  %02x:%02x '%c'",eeprom_cmdbuf[0],eeprom_cmdbuf[1],eeprom_cmdbuf[2],eeprom_cmdbuf[2]));
		...
		case 0xa1:
			switch(eeprom_cmdpos)
			{
				case 1:
					if(eeprom_mode!=EEPROM_INSEQ) 
						break;
LOG(("eeprom CMDA1     (%02x) %02x",eeprom_cmdbuf[0],eeprom_cmdbuf[1]));
				...
				default:
					if(eeprom_mode!=EEPROM_INCA1_DATA) 
						break;
LOG(("eeprom CMDA1 data [%04x]:%02x '%c'",eeprom_readpos>>3,eeprom_data_readbyte(),eeprom_data_readbyte()));
					break;
			}
			break;
		default:
...
//				}
			...
			if(ddr) /* ddr:write */
			{
				if(clk)
				{
					if(eeprom_clk)
					{
						/* clock already high, do nothing */
					}
					else
					{
						/* clock was low, write a bit */
						eeprom_data_write(data);
					}
					eeprom_clk=1;
				}
				else
...
//    mmcreplay_reset();
  ...
  else
  {
    eeprom_image_filename="./mmceeprom.bin";
      LOG(("eeprom card image name not set, using default: %s",eeprom_image_filename));
  }
