/*
 * via1d1990.c - VIA emulation in the 4000 disk drive.
 *
 * Written by
...
void via1d1990_store(drive_context_t *ctxptr, WORD addr, BYTE data)
{
    viacore_store(ctxptr->via1d1990, addr, data);
}

BYTE via1d1990_read(drive_context_t *ctxptr, WORD addr)
{
    return viacore_read(ctxptr->via1d1990, addr);
}

BYTE via1d1990_peek(drive_context_t *ctxptr, WORD addr)
{
    return viacore_peek(ctxptr->via1d1990, addr);
}
...
void via1d1990_init(drive_context_t *ctxptr)
{
    viacore_init(ctxptr->via1d1990, ctxptr->cpu->alarm_context,
                 ctxptr->cpu->int_status, ctxptr->cpu->clk_guard);
}

void via1d1990_setup_context(drive_context_t *ctxptr)
{
    drivevia_context_t *viap;
    via_context_t *via;

    /* Clear struct as snapshot code may write uninitialized values.  */
    ctxptr->via1d1990 = lib_calloc(1, sizeof(via_context_t));
    via = ctxptr->via1d1990;
    ...
    via->myname = lib_msprintf("1990Drive%dVia1", ctxptr->mynumber);
    via->my_module_name = lib_msprintf("1990VIA1D%d", ctxptr->mynumber);

    viacore_setup_context(via);

    via->my_module_name_alt1 = lib_msprintf("VIA1D%d", ctxptr->mynumber);
    via->my_module_name_alt2 = lib_msprintf("VIA1D1990");
