...
static void video_resources_update_ui(struct raster_s *canvas);

struct video_resource_chip_mode_s {
    struct raster_s *resource_chip;
    unsigned int device;
};
...
static int set_double_size_enabled(int val, void *param)
{
    ...
    raster_t *canvas = (raster_t *)param;
...
static int set_double_scan_enabled(int val, void *param)
{
    ...
    video_canvas_refresh_all(canvas->canvas);
...
static int set_fullscreen_enabled(int val, void *param)
{
    ...
    canvas->videoconfig->fullscreen_enabled = val;
	  ...
	  if (val)
	  {
	      r = (video_chip_cap->fullscreen.enable)(canvas->canvas, val);
	      (void) (video_chip_cap->fullscreen.statusbar)
		      (canvas->canvas, canvas->videoconfig->fullscreen_statusbar_enabled); 
    }
    else
    {
        /* always show statusbar when coming back to window mode */
        (void) (video_chip_cap->fullscreen.statusbar) (canvas->canvas, 1); 
        r = (video_chip_cap->fullscreen.enable)(canvas->canvas, val);
    }
    return r;
}
...
static int set_fullscreen_statusbar(int val, void *param)
{
    ...
    return (video_chip_cap->fullscreen.statusbar)(canvas->canvas, val);
}
...
static int set_fullscreen_double_size_enabled(int val, void *param)
{
    ...
    return (video_chip_cap->fullscreen.double_size)(canvas->canvas, val);
}
...
static int set_fullscreen_double_scan_enabled(int val, void *param)
{
    ...
    return (video_chip_cap->fullscreen.double_scan)(canvas->canvas, val);
}
...
static int set_fullscreen_device(const char *val, void *param)
{
    ...
    return (video_chip_cap->fullscreen.device)(canvas->canvas, val);
}
...
static int set_fullscreen_mode(int val, void *param)
{
    ...
    raster_t *canvas = video_resource_chip_mode->resource_chip;
    video_chip_cap_t *video_chip_cap = canvas->videoconfig->cap;
    ...
    return (video_chip_cap->fullscreen.mode[device])(canvas->canvas, val);
}
...
void video_resources_chip_shutdown(raster_t *canvas)
{
    lib_free(canvas->videoconfig->external_palette_name);
}

static void video_resources_update_ui(raster_t *canvas)
{
    int pal_enabled = 0;
