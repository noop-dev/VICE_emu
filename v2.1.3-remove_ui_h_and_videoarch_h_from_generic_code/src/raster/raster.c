...
static int raster_draw_buffer_alloc(draw_buffer_t *draw_buffer,
                                    ...
                                    unsigned int *fb_pitch)
{
    /* FIXME: Allocate one more line to prevent access violations by the
       scale2x render.  */
    draw_buffer->draw_buffer = (BYTE *)lib_malloc(fb_width
                                                          * (fb_height + 1));
    *fb_pitch = fb_width;
    return 0;
}

static void raster_draw_buffer_free(draw_buffer_t *draw_buffer)
{
    lib_free(draw_buffer->draw_buffer);
}

static void raster_draw_buffer_clear(draw_buffer_t *draw_buffer, BYTE value,
                                     unsigned int fb_width,
                                     unsigned int fb_height,
                                     unsigned int fb_pitch)
{
    memset(draw_buffer->draw_buffer, value, fb_width * fb_height);
}
...
static int raster_realize_frame_buffer(raster_t *raster)
{
    ...
    if (fb_width > 0 && fb_height > 0) {
        if (raster_draw_buffer_alloc(raster->draw_buffer, fb_width, fb_height,
            &fb_pitch))
        return -1;
        ...
        raster_draw_buffer_clear(raster->draw_buffer, 0, fb_width, fb_height,
                                 fb_pitch);
    }
                         ...
                         unsigned int extra_offscreen_border_right)
{
    ...
    raster->canvas_width = canvas_width;
    raster->canvas_height = canvas_height;
}
