...
int drive_snapshot_write_module(snapshot_t *s, int save_disks, int save_roms)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (0
            ...
            || SMW_B(m, (BYTE)(drive->image->diskID[0])) < 0
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (major_version == 1 && minor_version == 0) {
            if (0
                ...
                || SMR_B(m, &(drive->image->diskID[0])) < 0
...
static int drive_snapshot_read_gcrimage_module(snapshot_t *s, unsigned int dnr)
{
    ...
    if (SMR_W_INT(m, &drive->raw->size) < 0) {

        snapshot_module_close(m);
        return -1;
    }
    if (drive->raw->size > NUM_MAX_BYTES_TRACK) {
        drive->raw->size = NUM_MAX_BYTES_TRACK;
    }
