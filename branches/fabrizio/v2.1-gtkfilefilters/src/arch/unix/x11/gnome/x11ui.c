...
{  
	...
	GtkFileFilter *ff = gtk_file_filter_new();
        ...
        for (j = 0; filters[patterns[i]].filters[j] != NULL; j++)
            gtk_file_filter_add_pattern(ff, filters[patterns[i]].filters[j]);
	gtk_file_filter_set_name(ff, filters[patterns[i]].filter_name);
