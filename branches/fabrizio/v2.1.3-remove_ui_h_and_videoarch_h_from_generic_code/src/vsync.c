...
static void vsyncarch_verticalblank(struct video_canvas_s *c, float rate, int frames);

/* This is called at the end of each screen frame. It flushes the
   audio buffer and keeps control of the emulation speed. */
int vsync_do_vsync(struct video_canvas_s *c, float refreshrate, int been_skipped)
{
...
#if defined(WIN32) || defined(HAVE_OPENGL_SYNC)
    refresh_cmp = (float)(refreshrate / refresh_frequency);
    ...
    if ((timer_speed == 100) && (!warp_mode_enabled) &&
        ...
        (refresh_div == 1)) {
        vsyncarch_verticalblank(c, refreshrate, refresh_div);
        skip_next_frame = 0;
        skipped_redraw = 0;
    } else {
#endif
