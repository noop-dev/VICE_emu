/*
 * mmcreplay.c - Cartridge handling, Retro Replay cart.
 *
 * Written by
...
static unsigned int enable_clockport=0;
...
/********************************************************************************************************************/
 /* FIXME */
#include "ser-eeprom.c" 
#include "par-flashrom.c" 
...
static char ndumpstr[0x100];
    ...
    LOG(("MMCREPLAY: GAME:%d           EXROM:%d",enable_game,enable_exrom));	
     LOG(("MMCREPLAY: rr active: %d  rr regs:%d        mmc bios:%d",rr_active,enable_rr_regs,disable_mmc_bios));	
    LOG(("MMCREPLAY: extended mode:%d  16k mapping:%d",enable_extended_mode,enable_16k_mapping));	
     LOG(("MMCREPLAY: disable rom write:%d",disable_rom_write));	
    LOG(("MMCREPLAY: ROM Addr: %02x (%d:%d)",(bank_address_16_18<<3)|bank_address_13_15,bank_address_16_18,bank_address_13_15));	
    LOG(("MMCREPLAY: effective ROM Bank LO: %02x (%d:%d)",roml_bank,roml_bank>>3,roml_bank&7));	
    LOG(("MMCREPLAY: effective ROM Bank HI: %02x (%d:%d)",romh_bank,romh_bank>>3,romh_bank&7));	
    LOG(("MMCREPLAY: RAM REU Mapping:%d",enable_ram_io1));	
    LOG(("MMCREPLAY: effective RAM Bank LO: %02x (%d:%d)",raml_bank,raml_bank>>3,raml_bank&7));	
    LOG(("MMCREPLAY: effective RAM Bank HI: %02x (%d:%d)",ramh_bank,ramh_bank>>3,ramh_bank&7));	
#endif
#if 0			     
    {
	    char str[0x100];
	    strcpy(str,"ROML: ");
	    if(enable_raml) strcat(str,"RAM");
	    else strcat(str,"ROM");
	    
	    strcat(str," ROMH: ");
	    if(enable_ramh) strcat(str,"RAM");
	    else strcat(str,"ROM");
	    if(enable_ram_io1) strcat(str," RAMIO1: ");
	    else strcat(str," RAMIO2: ");
...
//    LOG(("MMCREPLAY: cart active: %d rr rom active:%d rr regs active:%d  extended mode:%d  16k mapping:%d RAM IO1 Mapping:%d RAM Banking:%d",rr_active,disable_rr_rom^1,enable_rr_regs,enable_extended_mode,enable_16k_mapping,enable_ram_io1,allow_bank));	
        sprintf(ndumpstr,"MMCREPLAY: cart active: %d disable rr rom/opt.mapping:%d rr regs active:%d  extended mode:%d  16k mapping:%d RAM IO1 Mapping:%d RAM Banking:%d",rr_active,disable_rr_rom,enable_rr_regs,enable_extended_mode,enable_16k_mapping,enable_ram_io1,allow_bank);	
...
//	    LOG(("MMCREPLAY: [%s] %s %s",str_mapper[mapper],str_config[config],str));	
        sprintf(ndumpstr,"MMCREPLAY: [%s] %s %s",str_mapper[mapper],str_config[config],str);	
...
//    export_ram = (mode_phi2 >> 5) & 1;
	...
	{
#ifdef DEBUG				
if(last_mainmode!=111){LOG(("main mode: mmc bios"));last_mainmode=111;}
#endif
		/**************************************************************************************************
			mmc bios mapper
		***************************************************************************************************/
		...
		*/
		if(disable_rr_rom) /* 1 = $8000 - $9fff	(512K read only  window) */
		{
			...
			{
#ifdef DEBUG				
if(last_biosmode!=111){LOG(("bios mode: 111"));last_biosmode=111;}
#endif
		/*   
...
//					enable_raml=0;
			...
			{
#ifdef DEBUG				
if(last_biosmode!=222){LOG(("bios mode: 222"));last_biosmode=222;}
#endif
		/*
		  disable_rr_rom=1 (opt.banking)
...
//					enable_raml=0;
		...
		else /* disable_rr_rom=0   $e000 - $ffff	(512K read/write window) */
		{
			...
			{
#ifdef DEBUG				
if(last_biosmode!=333){LOG(("bios mode: 333"));last_biosmode=333;}
#endif
		/*   
		...
		*/
				if(enable_ram_io) /* ultimax */
				{
					...
					enable_io1=1;
					enable_io2=1;
					mapped_game=1;mapped_exrom=0; /* ultimax */
					if(allow_bank)
					{
						io1bank_ram=(7<<3)|io1bank_ram;
						io2bank_ram=(7<<3)|io2bank_ram;
			...
			{
#ifdef DEBUG				
if(last_biosmode!=444){LOG(("bios mode: 444"));last_biosmode=444;}
#endif
		/*
		  disable_rr_rom=0 (opt.banking)
		  enable_ram_io1=0  (use io2 only)
...
//LOG(("aaa"));				
#ifdef DEBUG				
if(last_biosmode444!=111){LOG(("bios mode 444: ram enabled"));last_biosmode444=111;}
#endif
					/* ultimax, ram at $e000, rom at $8000, rom at $a000*/
					enable_ramh=1;
					enable_io1=0;
...
//					if(enable_extended_mode)
					{
						/* full rom banking */
						cartbankl=((bank_address_16_18<<3)|bank_address_13_15)&(0x3f);
						cartbankh=cartbankl;
						rambankl=((bank_address_16_18<<3)|bank_address_13_15)&(0x3f);
						rambankh=rambankl;
						io1bank=((bank_address_16_18<<3)|bank_address_13_15)&(0x3f);
						io2bank=io1bank;
...
//LOG(("this"));									
						...
						io1bank_ram=((bank_address_16_18<<3)|bank_address_13_15)&(0x3f);
						io2bank_ram=io1bank;
						}
						else
						{
...
//LOG(("bbb"));				
#ifdef DEBUG				
if(last_biosmode444!=222){LOG(("bios mode 444: ram disabled"));last_biosmode444=222;}
#endif
					enable_io1=0;
					enable_io2=1;
					mapped_game=1;mapped_exrom=1; /* 16k game */
...
//LOG(("that"));									
	...
	{
#ifdef DEBUG				
if(last_mainmode!=222){LOG(("main mode: super mapper"));last_mainmode=222;}
#endif
		/*************************************************************************************************
			super mapper
		**************************************************************************************************/
		...
		{
			case 0:
		mapped_game=0;mapped_exrom=1; /*0 0 8k game   roml ($8000) */
				break;
			case 1:
		mapped_game=1;mapped_exrom=1; /*1 0 16k game */
				break;
			case 2:
		mapped_game=0;mapped_exrom=1; /*0 1 8k game   roml ($8000) */
				break;
			case 3:
		mapped_game=1;mapped_exrom=0; /*1 1 ultimax   roml ($8000),romh ($e000) */
				break;
		}
		LOG(("mapped_game:%d mapped_exrom:%d",mapped_game,mapped_exrom));
#endif		
	...
	else if(disable_rr_rom)
	{
		...
		***************************************************************************************************/
#ifdef DEBUG				
if(last_mainmode!=333){LOG(("main mode: normal mapper"));last_mainmode=333;}
#endif
	...
	else
	{
		...
		***************************************************************************************************/
#ifdef DEBUG				
if(last_mainmode!=444){LOG(("MMCREPLAY: *** main mode: [rr mapper]"));last_mainmode=444;}
#endif
		cartbankl=(bank_address_16_18<<3)|(bank_address_13_15);
		cartbankh=cartbankl;
		rambankl=((bank_address_16_18<<3)|bank_address_13_15);
...
//				enable_io2=1;
	...
	mmcreplay_config_changed(active_mode_phi1,active_mode_phi2,wflag,release_freeze);
...
//    LOG(("MMCREPLAY: IO1 RD %04x",addr));	
	...
	*/
		...
		if(disable_mmc_bios==0)
		{
		    LOG(("MMCREPLAY: MAGIC IO1 RD %04x %02x",addr,0));	
			    return 0;
		}
            break;
          case 3:/* $DE03: */
		...
		if(disable_mmc_bios==0)
		{
			BYTE val=0x0e; // first magic
			    LOG(("MMCREPLAY: MAGIC IO1 RD %04x %02x",addr,val));
				    return val;
		}
            break;
          case 5:/* $DE05: */
...
//		    LOG(("read RAM IO1 %04x %02x (%02x:%04x)", addr,export_ram0[(addr & 0x1fff) + (io1_ram_bank << 13)],io1_ram_bank,(addr & 0x1fff)));
         	    return export_ram0[(io1_ram_bank << 13) + 0x1e00 + (addr & 0xff)];
                }
//		LOG(("read ROM IO1 %04x %02x (%02x:%04x)", addr,roml_banks[(addr & 0x1fff) + (io1_bank << 13)],io1_bank,(addr & 0x1fff)));
	    ...
	    {
		LOG(("read DISABLED IO1 %04x %02x (%02x:%04x)", addr,roml_banks[(addr & 0x1fff) + (io1_bank << 13)],io1_bank,(addr & 0x1fff)));
	    }
    } else {
...
#else				     
	LOG(("read RR DISABLED IO1 %04x %02x (%02x:%04x)", addr,roml_banks[(addr & 0x1fff) + (io1_bank << 13)],io1_bank,(addr & 0x1fff)));
#endif	    
...
//LOG(("MMCREPLAY: IO1 ST %04x %02x",addr,value));
    ...
    LOG(("MMCREPLAY: IO1 ST %04x enable_ram_io %02x",addr,enable_ram_io));	
...
//          mmcreplay_config_changed((enable_exrom<<1)|enable_game, (enable_exrom<<1)|enable_game, CMODE_WRITE);
	    ...
	    return;
		  ...
		  /* Every bit in $de01 can always be altered in Super Mapper mode+MMC Bios Mode. */
            if ((write_once == 0) || (enable_16k_mapping == 1) || (disable_mmc_bios == 0) )
	    {
	    bank_address_13_15=(((value >> 3) & 3) | ((value >> 5) & 4)); /* bit 3,4,7*/
	    	enable_clockport=value&1;  /* bit 0 */
                allow_bank = (value>>1) & 1;  /* bit 1 */
                no_freeze = (value>>2) & 1; /* bit 2 */
                enable_ram_io1 = (value>>6)&1; /* bit 6 */
		if((enable_extended_mode == 1) || (enable_16k_mapping == 1))
		{
			/*bit 5:	0 = enable MMC registers, 1 = disable MMC registers (W) */
...
//            mmcreplay_config_changed((enable_exrom<<1)|enable_game, (enable_exrom<<1)|enable_game, CMODE_WRITE);
    ...
    LOG(("MMCREPLAY: MAGIC IO1 ST %04x %02x",addr,value));	
	    ...
	    {
                if (enable_ram_io) {
		    LOG(("store RAM IO1 %04x %02x (%02x:%04x)", addr,value,io1_ram_bank,(addr & 0x1fff)));
		    export_ram0[(io1_ram_bank << 13) + 0x1e00 + (addr & 0xff)]=value;
                }
 	        else
		{
		    LOG(("store DISABLED RAM IO1 %04x %02x (%02x:%04x)", addr,value,io1_ram_bank,(addr & 0x1fff)));
		}
	    }
	    else
	    {
		LOG(("store DISABLED IO1 %04x %02x (%02x:%04x)", addr,value,io1_bank,(addr & 0x1fff)));
	    }
	    
    } /* rr active */
    else
    {
	LOG(("store DISABLED RR IO1 %04x %02x (%02x:%04x)", addr,value,io1_bank,(addr & 0x1fff)));
    }
}
...
//		LOG(("read ROM IO2 %04x %02x (%02x:%04x)", addr,roml_banks[(addr & 0x1fff) + (io2_bank << 13)],io2_bank,(addr & 0x1fff)));
	    ...
	    {
		LOG(("read DISABLED IO2 %04x %02x (%02x:%04x)", addr,roml_banks[(addr & 0x1fff) + (io2_bank << 13)],io2_bank,(addr & 0x1fff)));
	    }
//        return 0;
	 return vicii_read_phi1();
    } else {
	    
		LOG(("read RR DISABLED IO2 %04x %02x (%02x:%04x)", addr,roml_banks[(addr & 0x1fff) + (io2_bank << 13)],io2_bank,(addr & 0x1fff)));
...
//    LOG(("MMCREPLAY: IO2 ST %04x %02x",addr,value));	
    ...
    LOG(("MMCREPLAY: IO2 ST %04x (disabled!)",addr));
	    ...
	    case 0x11: /* OK $df11 MMC control register */
		    ...
		    {
    LOG(("MMCREPLAY: IO2 ST %04x %02x",addr,value));
    LOG(("MMCREPLAY: IO2 ST %04x  spi_mmc_card_selected %02x",addr,((value>>1)^1)&1));
...
//   LOG(("MMCREPLAY: IO2 ST %04x %02x",addr,value));
			    ...
			    if(enable_extended_mode)rr_active=1;
...
//    LOG(("MMCREPLAY: IO2 ST %04x (eeprom)",addr));
				/*
					...
					bit 7: clk
				*/
				eeprom_port_write((value>>7)&1,(value>>5)&1,(value>>1)&1,(value>>4)&1);
			    }
...
//    LOG(("MMCREPLAY: IO2 ST %04x (eeprom, reg disabled?)",addr));
    ...
    LOG(("MMCREPLAY: IO2 ST %04x bank_address_16_18 %02x",addr,bank_address_16_18));	
...
//            mmcreplay_config_changed((enable_exrom<<1)|enable_game, (enable_exrom<<1)|enable_game, CMODE_WRITE);
	    mmcreplay_update_mapper(CMODE_WRITE,0);
		    ...
		    {
#ifdef DEBUG
			    if(store_io2_warn<10){LOG(("MMCREPLAY: IO2 ST %04x %02x (disabled!)",addr,value));store_io2_warn++;}	
#endif	    
		    }
	    ...
	    if (enable_io2)
	    {
                if (enable_ram_io)
		{
		    LOG(("store RAM IO2 %04x %02x (%02x:%04x)", addr,value,io2_ram_bank,(addr & 0x1fff)));
		    export_ram0[(io2_ram_bank << 13) + 0x1f00 + (addr & 0xff)]=value;
                }
 	        else
		{
#ifdef DEBUG
		     if(store_io2_warn<10){LOG(("store DISABLED RAM in IO2 %04x %02x (%02x:%04x)", addr,value,io2_ram_bank,(addr & 0x1fff)));store_io2_warn++;}
#endif	    
		}
	    }
	    else
	    {
#ifdef DEBUG
		if(store_io2_warn<10){LOG(("store DISABLED IO2 %04x %02x (%02x:%04x)", addr,value,io2_bank,(addr & 0x1fff)));store_io2_warn++;}
#endif	    
	    }
...
//		LOG(("MMCREPLAY: RD %04x",addr));
		if (enable_raml)
		{
			LOG(("MMCREPLAY: RAML RD %04x %02x (%02x:%04x)", addr,export_ram0[(addr & 0x1fff) + (raml_bank << 13)],raml_bank,(addr & 0x1fff)));
			logbank_read=(addr&0xe000);
		}
		else
		{
...
void REGPARM2 mmcreplay_roml_store(WORD addr, BYTE value)
{
	...
	if(logbank_write!=(addr&0xe000))
	{
		...
		if (enable_raml)
		{
			LOG(("MMCREPLAY: RAML ST %04x %02x (%02x:%04x)", addr,value,raml_bank,(addr & 0x1fff)));
		}
		else
		{
			LOG(("MMCREPLAY: DISABLED! RAML ST  %04x %02x (%02x:%04x)", addr,value,raml_bank,(addr & 0x1fff)));
		}
		logbank_write=(addr&0xe000);
	}
#endif
...
{
#ifdef DEBUG_LOGBANKS
	if(logbank_write!=(addr&0xe000)){LOG(("MMCREPLAY: ST %04x",addr));logbank_write=(addr&0xe000);}
#endif
    if(!disable_mmc_bios)
    {
	    if (enable_ramA000)
	    {
		    LOG(("store RAM A000 %04x %02x (%02x:%04x)", addr,value,ramA000_bank,(addr & 0x1fff)));
		    export_ram0[(addr & 0x1fff) + (ramA000_bank << 13)] = value;
	    }
	    else
	    {
		    LOG(("store DISABLED RAM A000 %04x %02x (%02x:%04x)", addr,value,romA000_bank,(addr & 0x1fff)));
//		    export_ram0[(addr & 0x1fff) + (ramA000_bank << 13)] = value;
	    }
    }
}
...
BYTE REGPARM1 mmcreplay_romh_read(WORD addr)
{
	...
	if(logbank_read!=(addr&0xe000))
	{
		if (enable_ramh)
		{
			if(addr<0xfff0)
			{
				LOG(("MMCREPLAY: RAMH RD %04x %02x (%02x:%04x)", addr,export_ram0[(addr & 0x1fff) + (ramh_bank << 13)],ramh_bank,(addr & 0x1fff)));
				logbank_read=(addr&0xe000);
			}
		}
		else
		{
			LOG(("MMCREPLAY: ROMH RD %04x %02x (%02x:%04x)", addr,romh_banks[(addr & 0x1fff) + (romh_bank << 13)],romh_bank,(addr & 0x1fff)));
			logbank_read=(addr&0xe000);
		}
//		LOG(("MMCREPLAY: RD %04x",addr));
...
void REGPARM2 mmcreplay_romh_store(WORD addr, BYTE value)
{
	...
	if(logbank_write!=(addr&0xe000))
	{
		if (enable_ramh)
		{
			LOG(("MMCREPLAY: RAMH ST %04x %02x (%02x:%04x)", addr,value,ramh_bank,(addr & 0x1fff)));
		}
		else
		{
			LOG(("MMCREPLAY: DISABLED! RAMH ST %04x %02x (%02x:%04x)", addr,value,ramh_bank,(addr & 0x1fff)));
		}
		logbank_write=(addr&0xe000);
	}
#endif
