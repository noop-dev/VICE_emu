...
int CALLBACK about_dialog_proc(HWND dialog, UINT msg, UINT wparam, LONG lparam)
{
    ...
    switch (msg) {
      case WM_INITDIALOG:
#ifdef UNSTABLE
        version = lib_msprintf(intl_translate_text_new(IDS_VERSION_S_UNSTABLE), VERSION);
#else /* #ifdef UNSTABLE */
        version = lib_msprintf(intl_translate_text_new(IDS_VERSION_S), VERSION);
...
void uihelp_dialog(HWND hwnd, WPARAM wparam)
{
    ...
    switch (wparam) {
      ...
      case IDM_CONTRIBUTORS:
        ui_show_text(hwnd, intl_translate_text_new(IDS_VICE_CONTRIBUTORS), intl_translate_text_new(IDS_WHO_MADE_WHAT),
                     info_contrib_text);
        break;
      case IDM_LICENSE:
        ui_show_text(hwnd, intl_translate_text_new(IDS_LICENSE),
                     "VICE license (GNU General Public License)",
                     info_license_text);
        break;
      case IDM_WARRANTY:
        ui_show_text(hwnd, intl_translate_text_new(IDS_NO_WARRANTY),
                     intl_translate_text_new(IDS_VICE_WITHOUT_WARRANTY),
                     info_warranty_text);
        break;
      case IDM_CMDLINE:
