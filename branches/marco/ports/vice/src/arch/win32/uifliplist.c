...
static void uifliplist_load_dialog(HWND hwnd)
{
    ...
    if ((st_name = uilib_select_file(hwnd, intl_translate_text_new(IDS_LOAD_FLIP_LIST_FILE),
        ...
        if (fliplist_load_list((unsigned int)-1, name, 0) != 0)
            ui_error(intl_translate_text_new(IDS_CANNOT_READ_FLIP_LIST));
...
static void uifliplist_save_dialog(HWND hwnd)
{
    ...
    if ((st_name = uilib_select_file(hwnd, intl_translate_text_new(IDS_SAVE_FLIP_LIST_FILE),
        ...
        if (fliplist_save_list((unsigned int)-1, name) != 0)
            ui_error(intl_translate_text_new(IDS_CANNOT_WRITE_FLIP_LIST));
        system_wcstombs_free(name);
        lib_free(st_name);
    }
}
