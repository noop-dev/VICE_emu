	...
	while ((!stopPoll) && Android_PollEvent(&event1))
	{
		...
		switch (event->eventType) {
			...
			case SDL_JOYAXISMOTION:
			{
				...
				if (y < -DEAD_ZONE) top = 1;
				if (y > DEAD_ZONE) bot = 1;
				if (x < -DEAD_ZONE) left = 1;
				if (x > DEAD_ZONE) right = 1;
				...
				/*}
					...
					joystick_set_value_absolute(loader_joyport,value);
				}*/
			...
			case SDL_JOYBUTTONDOWN:
			{
				...
				//else
				//	joystick_set_value_absolute(loader_joyport,old_joy_direction|16);
			...
			case SDL_JOYBUTTONUP:
			{
				...
				//else
				//	joystick_set_value_absolute(loader_joyport,old_joy_direction);
			...
			case SDL_KEYDOWN:
			{
				...
				int kmod = 0;
				if (ctrl) kmod |= KMOD_LCTRL;
				if (alt) kmod |= KMOD_LALT;
				if (shift) kmod |= KMOD_LSHIFT;
//	__android_log_print(ANDROID_LOG_INFO, LOCNET_LOG_TAG, "event : %d, %d, %d, %d, %d", down, kcode, ctrl, alt, shift);

				if (down) {
					//if (ctrl) keyboard_key_pressed((unsigned long)SDLK_LCTRL);
					if (ctrl || (kcode == SDLK_TAB)) {
						if (!ctrl_down)
							keyboard_key_pressed((unsigned long)SDLK_TAB);
						ctrl_down++;						
					}
					//if (alt) keyboard_key_pressed((unsigned long)SDLK_LALT);
					...
					//if (kcode != KEYBOARD_NONE)
					//	keyboard_key_pressed((unsigned long)kcode);
					//retval = sdlkbd_press(kcode, kmod);
					retval = sdlkbd_press(kcode, 0);
				}
				else {
					//retval = sdlkbd_release(kcode, kmod);
					retval = sdlkbd_release(kcode, 0);

					//if (kcode != KEYBOARD_NONE)
					//	keyboard_key_released((unsigned long)kcode);

					//if (ctrl) keyboard_key_released((unsigned long)SDLK_LCTRL);
					if (ctrl || (kcode == SDLK_TAB)) {
						if (kcode == SDLK_TAB)
							ctrl_down = 0;
						if (ctrl_down)
					...
					//if (alt) keyboard_key_released((unsigned long)SDLK_LALT);
					if (alt || (kcode == SDLK_LCTRL)) {
						if (kcode == SDLK_LCTRL)
							alt_down = 0;
						if (alt_down)
