/*
 * mon_registerWDC65C02.c - The VICE built-in monitor WDC65C02 register functions.
 *
 * Written by
...
static unsigned int mon_register_get_val(int mem, int reg_id)
{
    ...
    reg_ptr = mon_interfaces[mem]->cpu_WDC65C02_regs;

    switch(reg_id) {
      case e_A:
        return WDC65C02_REGS_GET_A(reg_ptr);
      case e_X:
        return WDC65C02_REGS_GET_X(reg_ptr);
      case e_Y:
        return WDC65C02_REGS_GET_Y(reg_ptr);
      case e_PC:
        return WDC65C02_REGS_GET_PC(reg_ptr);
      case e_SP:
        return WDC65C02_REGS_GET_SP(reg_ptr);
      case e_FLAGS:
          return WDC65C02_REGS_GET_FLAGS(reg_ptr)
              | WDC65C02_REGS_GET_SIGN(reg_ptr)
              | (WDC65C02_REGS_GET_ZERO(reg_ptr) << 1);
      default:
...
static void mon_register_set_val(int mem, int reg_id, WORD val)
{
    ...
    switch(reg_id) {
      case e_A:
        WDC65C02_REGS_SET_A(reg_ptr, (BYTE)val);
        break;
      case e_X:
        WDC65C02_REGS_SET_X(reg_ptr, (BYTE)val);
        break;
      case e_Y:
        WDC65C02_REGS_SET_Y(reg_ptr, (BYTE)val);
        break;
      case e_PC:
        WDC65C02_REGS_SET_PC(reg_ptr, val);
        if (monitor_diskspace_dnr(mem) >= 0) {
            mon_interfaces[mem]->set_bank_base(mon_interfaces[mem]->context);
        }
        break;
      case e_SP:
        WDC65C02_REGS_SET_SP(reg_ptr, (BYTE)val);
        break;
      case e_FLAGS:
        WDC65C02_REGS_SET_STATUS(reg_ptr, (BYTE)val);
        break;
      default:
        log_error(LOG_ERR, "Unknown register!");
        return;
...
static void mon_register_print(int mem)
{
    ...
    regs = mon_interfaces[mem]->cpu_WDC65C02_regs;
    ...
    mon_out(".;%04x %02x %02x %02x %02x %02x %02x %d%d%c%d%d%d%d%d",
              ...
              TEST(WDC65C02_REGS_GET_SIGN(regs)),
              TEST(WDC65C02_REGS_GET_OVERFLOW(regs)),
              '1',
              TEST(WDC65C02_REGS_GET_BREAK(regs)),
              TEST(WDC65C02_REGS_GET_DECIMAL(regs)),
              TEST(WDC65C02_REGS_GET_INTERRUPT(regs)),
              TEST(WDC65C02_REGS_GET_ZERO(regs)),
              TEST(WDC65C02_REGS_GET_CARRY(regs)));
...
static mon_reg_list_t *mon_register_list_getWDC65C02(int mem)
{
...
static void mon_register_list_setWDC65C02(mon_reg_list_t *reg_list, int mem)
{
    do {
...
void mon_registerWDC65C02_init(monitor_cpu_type_t *monitor_cpu_type)
{
    ...
    monitor_cpu_type->mon_register_list_get = mon_register_list_getWDC65C02;
    monitor_cpu_type->mon_register_list_set = mon_register_list_setWDC65C02;
}
