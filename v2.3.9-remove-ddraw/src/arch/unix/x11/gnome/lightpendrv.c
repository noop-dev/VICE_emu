...
void x11_lightpen_update(void)
{
    ...
    if (c && lightpen_enabled)  {
        gdk_window_set_cursor(c->canvas->emuwindow->window, cursor);
        gdk_display_get_window_at_pointer(
                gtk_widget_get_display(c->canvas->emuwindow), &x, &y);
        gdk_drawable_get_size(c->canvas->emuwindow->window, &w, &h);
        fx = w / (float) (c->geometry->screen_size.width - c->canvas->offx);
        ...
        if (c) {
            gdk_window_set_cursor(c->canvas->emuwindow->window, NULL);
        }
        buttons = 0;
    }
}

void gtk_lightpen_update_canvas(raster_t *p, int enter) 
{
    c = enter ? p : NULL;
}
