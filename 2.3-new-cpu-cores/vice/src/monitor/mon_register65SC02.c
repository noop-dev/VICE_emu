/*
 * mon_register65SC02.c - The VICE built-in monitor 65SC02 register functions.
 *
 * Written by
...
static unsigned int mon_register_get_val(int mem, int reg_id)
{
    ...
    switch(reg_id) {
      case e_A:
        return MOS65SC02_REGS_GET_A(reg_ptr);
      case e_X:
        return MOS65SC02_REGS_GET_X(reg_ptr);
      case e_Y:
        return MOS65SC02_REGS_GET_Y(reg_ptr);
      case e_PC:
        return MOS65SC02_REGS_GET_PC(reg_ptr);
      case e_SP:
        return MOS65SC02_REGS_GET_SP(reg_ptr);
      case e_FLAGS:
          return MOS65SC02_REGS_GET_FLAGS(reg_ptr)
              | MOS65SC02_REGS_GET_SIGN(reg_ptr)
              | (MOS65SC02_REGS_GET_ZERO(reg_ptr) << 1);
      default:
...
static void mon_register_set_val(int mem, int reg_id, WORD val)
{
    ...
    switch(reg_id) {
      case e_A:
        MOS65SC02_REGS_SET_A(reg_ptr, (BYTE)val);
        break;
      case e_X:
        MOS65SC02_REGS_SET_X(reg_ptr, (BYTE)val);
        break;
      case e_Y:
        MOS65SC02_REGS_SET_Y(reg_ptr, (BYTE)val);
        break;
      case e_PC:
        MOS65SC02_REGS_SET_PC(reg_ptr, val);
        if (monitor_diskspace_dnr(mem) >= 0) {
            mon_interfaces[mem]->set_bank_base(mon_interfaces[mem]->context);
        }
        break;
      case e_SP:
        MOS65SC02_REGS_SET_SP(reg_ptr, (BYTE)val);
        break;
      case e_FLAGS:
        MOS65SC02_REGS_SET_STATUS(reg_ptr, (BYTE)val);
        break;
      default:
        log_error(LOG_ERR, "Unknown register!");
        return;
...
static void mon_register_print(int mem)
{
    ...
    regs = mon_interfaces[mem]->cpu_65SC02_regs;
    ...
    mon_out(".;%04x %02x %02x %02x %02x %02x %02x %d%d%c%d%d%d%d%d",
              ...
              TEST(MOS65SC02_REGS_GET_SIGN(regs)),
              TEST(MOS65SC02_REGS_GET_OVERFLOW(regs)),
              '1',
              TEST(MOS65SC02_REGS_GET_BREAK(regs)),
              TEST(MOS65SC02_REGS_GET_DECIMAL(regs)),
              TEST(MOS65SC02_REGS_GET_INTERRUPT(regs)),
              TEST(MOS65SC02_REGS_GET_ZERO(regs)),
              TEST(MOS65SC02_REGS_GET_CARRY(regs)));
...
static mon_reg_list_t *mon_register_list_get65SC02(int mem)
{
...
static void mon_register_list_set65SC02(mon_reg_list_t *reg_list, int mem)
{
    do {
...
void mon_register65SC02_init(monitor_cpu_type_t *monitor_cpu_type)
{
    ...
    monitor_cpu_type->mon_register_list_get = mon_register_list_get65SC02;
    monitor_cpu_type->mon_register_list_set = mon_register_list_set65SC02;
}
